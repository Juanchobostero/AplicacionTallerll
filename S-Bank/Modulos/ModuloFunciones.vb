...
Module ModuloFunciones
    ...
    '***********************COMBOBOX CON CUENTAS ASOCIADAS A UN USUARIO **********************************
    ...
    Public Function CargarComboBoxUsuarios(ByVal comboBox As ComboBox, ByVal IDusuario As Integer) As Boolean
        Dim conectar As New System.Data.SqlClient.SqlConnection(ModuloConexion.source)
        Dim cmsql As New System.Data.SqlClient.SqlCommand
        Dim sql As String
        ...
        Try
            ...
            sql = "SELECT nick_usuario FROM usuarios WHERE id_usuario <> '" & IDusuario & "'"
            conectar.Open()
            cmsql = New SqlCommand(sql, conectar)
            Dim da As New SqlDataAdapter(cmsql)
            'crear y llenar el dateSet
    ...
    Public Function verificarServicio()
        If frmPagarServicios.cbxServicios.Text = GetElementFromGridViewCol("Servicio", frmPagarServicios.dgvDetalle) Then
            Return True
        End If
    ...
    Public Function existeCuentaEnMovimiento()

        'Extraer id_tipoCuenta
        Dim nombreCuenta As String = GetElementFromGridViewCol("Tipo Cuenta", frmAdministrarCuentas.dgvCuentas)
    ...
    Public Function existeCuentaEnFactura()

        'extraer id del tipo de cuenta 
        Dim descrip As String = GetElementFromGridViewCol("Tipo Cuenta", frmAdministrarCuentas.dgvCuentas)
    ...
    Public Function existeServicioEnDetalle() As Boolean
        ...
        Dim descripcionServicio As String = GetElementFromGridViewCol("Servicio", frmAdministrarServicios.dgvTipoServicio)
    ...
    Public Function verificarPeriodo()
        ...
        Dim tipoCuenta As String = frmPagarServicios.cbxCuentasAsociadas.Text
        Dim servicio As String = frmPagarServicios.cbxServicios.Text
        Dim usuario As String = frmPagarServicios.lblNomUsuario.Text
        ...
        Dim query2 As String = "SELECT s.descripcion_servicio FROM servicios s INNER JOIN detalleServicios ds ON ds.cod_servicio = s.cod_servicio INNER JOIN Factura f ON f.id_factura = ds.id_factura WHERE ds.id_factura ='" & IDfactura & "'  and s.descripcion_servicio ='" & servicio & "'"
        Dim descripcionServicio As String = ModuloConexion.SQL_Consulta2(ModuloConexion.sqlConnection1, query2)
        ModuloConexion.sqlConnection1.Close()

        Dim query3 As String = "SELECT periodoMes FROM Factura WHERE id_factura ='" & IDfactura & "'"
        Dim periodoMONTH As String = ModuloConexion.SQL_Consulta2(ModuloConexion.sqlConnection1, query3)
        ModuloConexion.sqlConnection1.Close()

        If periodoMes = periodoMONTH And servicio = descripcionServicio Then
            Return True
        Else
            Return False
        End If
    ...
    Public Function ConsultarSaldo2()
        ...
        If monto > saldoCuentaOrigen Then
            Return True
        Else
            Return False
        End If
    ...
    Public Function ConsultarSaldo3()
        Dim nickUsuario As String = frmTransferencias.lblNomUsuario.Text
    ...
    Public Function existeServicio() As Boolean

        Dim servicio As String = frmAdministrarServicios.txtDescripcionServicio.Text
        Dim proveedor As String = frmAdministrarServicios.cbxProveedor.Text

        Dim query As String = "SELECT s.descripcion_servicio FROM servicios s INNER JOIN proveedores p ON p.id_proveedor = s.id_proveedor WHERE s.descripcion_servicio = '" & servicio & "' and p.descripcion_proveedor ='" & proveedor & "'"
        Dim servicio2 As String = ModuloConexion.SQL_Consulta2(ModuloConexion.sqlConnection1, query)
        ModuloConexion.sqlConnection1.Close()

        If (servicio2 = String.Empty) Then
            Return False
        Else
            Return True
        End If
